<div class="date {{ Route::is('fuel_test_stats') ? 'chart-date' : '' }}">
    <h1 class="page-title">{{ $title }} <span>{{ Request::is('Vendors') ? '(' . $number_of_vendors_absolute . ') (' .  $number_of_vendors . ')' : '' }}{{ Request::is('Users') ? '(' . $number_of_fuel_test_users . ')' : '' }}</span> <br>
    <span>
        {{ Route::is('fuel_test') ? 'MANAGEMENT SYSTEM' : '' }}
        {{ !(Route::is('fuel_test')) ? 'FUEL TEST MANAGEMENT SYSTEM' : '' }}
    </span>
    </h1> 
    <div>  
        ...
        <form action="{{ route('all_records') }}">
            ...
            <section class="tooltip">Passed Records = [{{ $number_of_passed_records }}]</section> 
        </form>  
        <form action="{{ route('all_records') }}">
            <label class="LoaderElement">
            ...
            <section class="tooltip">Failed Records = [{{ $number_of_failed_records }}]</section> 
        </form>     
        <a href="{{ route('previous_records') }}">                                               
            ...
            <section class="tooltip">Previous Records = [{{ $number_of_previous_records }}]</section> 
        </a>    
        <a href="{{ route('all_records') }}">                        
            <p><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M0 64C0 28.65 28.65 0 64 0H224V128C224 145.7 238.3 160 256 160H384V448C384 483.3 355.3 512 320 512H64C28.65 512 0 483.3 0 448V64zM256 128V0L384 128H256z"/></svg>{{ $number_of_all_records_absolute }}</p> 
            <section class="tooltip">All Records = [{{ $number_of_all_records_absolute }}]</section> 
        </a>    
        <a href="{{ route('vendors') }}">                      
            <p><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M352 128C352 198.7 294.7 256 224 256C153.3 256 96 198.7 96 128C96 57.31 153.3 0 224 0C294.7 0 352 57.31 352 128zM209.1 359.2L176 304H272L238.9 359.2L272.2 483.1L311.7 321.9C388.9 333.9 448 400.7 448 481.3C448 498.2 434.2 512 417.3 512H30.72C13.75 512 0 498.2 0 481.3C0 400.7 59.09 333.9 136.3 321.9L175.8 483.1L209.1 359.2z"/></svg>{{ $number_of_vendors_absolute }}</p> 
            <section class="tooltip">Vendors = [{{ $number_of_vendors_absolute }}]</section> 
        </a>   
    </div> 
</div> 
